...
def loadData():
    # mir-mir net weight
    mirSim = np.load('data_cdhgnn/mirNet_weight.npy')
...
if __name__ == "__main__":
    ...
    # load interaction profile for miRNA
    mirInterProfile = np.load('data_cdhgnn/mirInterProfile.npy')
    # load centrality
    mirCentra = getCentralities()

    # initial features + structural features
    mir_merge = np.concatenate((np.around(mirInterProfile, decimals=2), mirCentra.reshape(-1, 1)), axis=1)

    # embedding = np.random.randn(20,12).astype('float32')
    index_tuple = np.where(mirSim >0 )
    ...
    while len(neg_set)<num_pos:
        row_rand = random.randint(0,num_mir-1)
        col_rand = random.randint(0,num_mir-1)
    ...
    features = torch.from_numpy(mir_merge).to(torch.float32).to(device)
    ...
    print("end training")
    torch.save(x, 'data_cdhgnn/mirFea.pt')
